...
public final class BundledImageRegistry {
    ...
    /** key → registered texture ResourceLocation */
     ...
     */
    public static Map<String, ResourceLocation> getAll() {
        if (!loaded) load();
    ...
    private static void load() {
        ...
        try {
            Map<ResourceLocation, Resource> resources = mc.getResourceManager()
            ...
            for (Map.Entry<ResourceLocation, Resource> entry : sorted) {
                ...
                try (InputStream is = entry.getValue().open()) {
                    ...
                    ResourceLocation texLoc = mc.getTextureManager()
                            .register("discordrpc/bundled/" + sanitized, tex);
    ...
    static NativeImage resample(NativeImage src, int tw, int th) {
        ...
        for (int dy = 0; dy < th; dy++) {
            ...
            for (int dx = 0; dx < tw; dx++) {
                ...
                out.setPixelRGBA(dx, dy, blendBilinear(c00, c10, c01, c11, tx, ty));
            }
        }
        return out;
    }
