...
public class DiscordPreviewWidget extends AbstractWidget {
    ...
    private ResourceLocation largeImageTex;
    private ResourceLocation smallImageTex;
    ...
    protected void renderWidget(GuiGraphics g, int mx, int my, float pt) {
        ...
        // Header row
        g.drawString(font, "PLAYING A GAME", x + pad, y + 6, C_HEADER, false);
        ...
        if (hasLargeImage) {
            ...
            if (largeImageTex != null) {
                g.blit(largeImageTex, imgX, cy, 0f, 0f, imgSize, imgSize, imgSize, imgSize);
            } else {
                fillRounded(g, imgX, cy, imgX + imgSize, cy + imgSize, C_IMG_BG);
                String q = "?";
                g.drawString(font, q, imgX + (imgSize - font.width(q)) / 2, cy + imgSize / 2 - 4, C_EMPTY, false);
            }
            ...
            if (hasSmallImage && smallImageTex != null) {
                g.blit(smallImageTex, smX, smY, 0f, 0f, smSize, smSize, smSize, smSize);
        ...
        g.drawString(font, Component.literal("Minecraft").withStyle(s -> s.withBold(true)), textX, cy, C_WHITE, false);

        int lineY = cy + 12;
        zoneRects.put(Zone.DETAILS, new int[]{textX - 2, lineY - 1, maxTW + 4, 11});
        if (!details.isEmpty()) {
            g.drawString(font, trimToWidth(font, details, maxTW), textX, lineY, C_BODY, false);
        } else {
            g.drawString(font, trimToWidth(font, emptyLabel(Zone.DETAILS), maxTW), textX, lineY, C_EMPTY, false);
        }
        lineY += 11;

        zoneRects.put(Zone.STATE, new int[]{textX - 2, lineY - 1, maxTW + 4, 11});
        if (!state.isEmpty()) {
            g.drawString(font, trimToWidth(font, state, maxTW), textX, lineY, C_BODY, false);
        } else {
            g.drawString(font, trimToWidth(font, emptyLabel(Zone.STATE), maxTW), textX, lineY, C_EMPTY, false);
        }
        lineY += 11;

        zoneRects.put(Zone.TIMESTAMP, new int[]{textX - 2, lineY - 1, maxTW + 4, 11});
        if (showTimestamp && !elapsedTime.isEmpty()) {
            g.drawString(font, elapsedTime, textX, lineY, C_MUTED, false);
        } else {
    ...
    public void setLargeImageTexture(ResourceLocation tex) { this.largeImageTex = tex; }
    public void setSmallImageTexture(ResourceLocation tex) { this.smallImageTex = tex; }
